...
abstract class _ImageIORasterGraphicDriver extends AbstractGraphicDriver {
          ...
          if (spiIt != null) {
            finderLoop: while (spiIt.hasNext()) {
              ...
              if (spi != null) {
                ...
                if (spi.getClass().getCanonicalName()
                    .contains("freehep")) { //$NON-NLS-1$
                  if ((emergency == null) || (spi
  ...
  protected final Graphic createGraphic(final GraphicBuilder builder) {
    ...
    wPt = Math.max(1, ((int) ((0.5d + //
        sizeUnit.convertToAsDouble(w, ELength.PT)))));
    hPt = Math.max(1, ((int) ((0.5d + //
        sizeUnit.convertToAsDouble(h, ELength.PT)))));

    wIn = ELength.PT.convertToAsDouble(((double) wPt), ELength.INCH);
    hIn = ELength.PT.convertToAsDouble(((double) hPt), ELength.INCH);
    ...
    return this._create(
        ...
        builder.getLogger(), builder.getFileProducerListener(), img, g,
        wPt, hPt, wDPI, hDPI, builder.getQuality());
  }
