...
public final class GraphicDriverParser
    extends InstanceParser<IGraphicDriver> {
  ...
  private static final String[] __prefixes() {
    ...
    ReflectionUtils.addPackageOfClassToPrefixList(EGraphicFormat.class,
        paths);
    ReflectionUtils
        .addPackageOfClassToPrefixList(FreeHEPGraphicDrivers.class, paths);
      ...
      try {
        ...
        RethrowMode.AS_UNSUPPORTED_OPERATION_EXCEPTION.rethrow(
            ((("Could not find graphic driver fitting to string '" //$NON-NLS-1$
                + string) + '\'') + '.'),
            false, ErrorUtils.aggregateError(exception, exception2));
